...
export default function PostcodeLookup({
  ...
  return (
    <div className={`space-y-3 ${className}`}>
      ...
      <div>
        ...
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 px-4 py-2.5 rounded-xl text-sm border border-[var(--border)] bg-[var(--input)] text-[var(--text)] placeholder:text-[var(--text-muted)] outline-none focus:border-[var(--primary)] transition uppercase tracking-wide"
          />
          <button
            type="button"
            onClick={handleLookup}
            disabled={loading}
            className="px-4 py-2.5 rounded-xl text-sm font-medium text-white transition disabled:opacity-50 flex items-center gap-2 shrink-0"
      ...
      {/* Error message */}
      {error && (
        <div className="flex items-center gap-2 text-sm rounded-xl px-3 py-2" style={{ color: "var(--alert)", background: "rgba(255,45,85,0.05)", border: "1px solid rgba(255,45,85,0.2)" }}>
      ...
      {validated && addresses.length === 0 && !error && (
        <div className="flex items-center gap-2 text-green-600 dark:text-green-400 text-sm bg-green-500/5 border border-green-500/20 rounded-xl px-3 py-2">
          ...
          <span>
            <strong>{lookupData?.postcode}</strong> — {lookupData?.city || "UK"}. Now enter your street address below.
          </span>
        </div>
      )}
        ...
        <div>
          ...
          <div className="relative">
            <select
              ...
              className="w-full px-4 py-2.5 rounded-xl text-sm border border-[var(--border)] bg-[var(--input)] text-[var(--text)] outline-none focus:border-[var(--primary)] transition appearance-none cursor-pointer"
            >
